...
function loadContent() {
    ...
    qtyElements.forEach((input) => {
        console.log('Attaching change event to cart-quantity:', input);
...
function addCart() {
    ...
    if (itemList.find((el) => el.title == newProduct.title)) {
        alert("هذه الوجبة موجودة بالقائمة فان اردت زيادة في العدد اضغط على العداد من داخل قائمة الطلبات");
        return;
    } else {
    ...
    return `
     <div class="cart-box">
        ...
        <div class="detail-box">
            ...
            <h5><input type="number" value="1" class="cart-quantity"> piece</h5>
        </div>
...
function updateTotal() {
    ...
    cartItems.forEach(product => {
        ...
        total += (price * qty);
        product.querySelector('.cart-amt').innerHTML = "Dnr." + (price * qty).toFixed(3);
    });

    // Convert total to string and remove '.00' if it exists
    let totalString = total.toFixed(3).replace(/\.00$/, '');
